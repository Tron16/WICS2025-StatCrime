...
df = pd.read_csv('data/Houston.csv')
df['hour'] = df['RMSOccurrenceHour']
df = df.dropna(subset=['MapLatitude', 'MapLongitude'])

coords = df[['MapLatitude', 'MapLongitude']].values

kde = KernelDensity(bandwidth=0.01, kernel='gaussian')
kde.fit(coords)

n_points = 200
lon_min, lon_max = df['MapLongitude'].min(), df['MapLongitude'].max()
lat_min, lat_max = df['MapLatitude'].min(), df['MapLatitude'].max()
...
# Hourly safety score for most dangerous location
def calculate_distance(row):
    crime_location = (row['MapLatitude'], row['MapLongitude'])
...
for h in sorted(df['hour'].unique()):
    ...
    if len(df_hour) < 5:
        ...
        continue
    coords_h = df_hour[['MapLatitude', 'MapLongitude']].values
